  ...
  const renderPodeGastarCard = () => (
    <div className="card-zelo fade-in-up relative overflow-hidden">
      ...
      <div className="flex items-end gap-3 mb-1 relative z-10">
        ...
        {hasData && !hideValues && (
          <span className={`mb-1 inline-flex items-center gap-1 rounded-full px-2.5 py-1 text-[10px] font-semibold ${isHealthy ? "bg-success/12 text-success border border-success/20" : "bg-warning/12 text-warning border border-warning/20"}`}>
      ...
      <div className="flex items-center gap-1.5 mb-3 relative z-10">
        ...
        {!hideValues && (
          <button onClick={() => setEditingPercent(!editingPercent)} className="flex h-5 w-5 items-center justify-center rounded-lg bg-muted/60 text-muted-foreground hover:text-foreground active:scale-90 transition-all duration-300">
            <Pencil size={10} />
          </button>
        )}
      </div>
      ...
      {editingPercent && (
        <div className="mb-3 rounded-2xl bg-muted/30 border border-border/50 p-3.5 space-y-2 fade-in-up relative z-10">
          <div className="flex items-center justify-between">
          ...
          <input type="range" min={5} max={50} step={5} value={spendPercent} onChange={(e) => setSpendPercent(Number(e.target.value))} className="w-full accent-primary h-1.5" />
      ...
      {!hideValues && (
        <div className="flex items-center gap-2 relative z-10">
          <button onClick={() => setSimOpen(true)} className="inline-flex items-center gap-1.5 rounded-full bg-primary/12 border border-primary/25 px-4 py-2 text-[11px] font-semibold text-primary cursor-pointer active:scale-95 transition-all duration-300 hover:bg-primary/18">
    ...
    <div className="space-y-3">
      ...
      {simOpen && (
        <div className="fixed inset-0 z-[60] flex items-end justify-center">
          ...
          <div className="relative w-full max-w-lg animate-slide-up rounded-t-3xl bg-card/90 backdrop-blur-xl border-t border-border/50 p-5 pb-8">
            <div className="flex items-center justify-between mb-4">
              ...
              <button onClick={() => setSimOpen(false)} className="rounded-full p-2 text-muted-foreground hover:text-foreground active:scale-95 transition-all duration-300"><X size={20} /></button>
            </div>
            ...
            <div className="flex items-center gap-2 mb-5">
              <div className="flex-1 relative">
                ...
                <input type="text" inputMode="numeric" value={simValue || ""} onChange={(e) => { const val = e.target.value.replace(/[^\d.,]/g, "").replace(",", "."); setSimValue(Math.max(0, Number(val) || 0)); }} placeholder="0,00" className="w-full rounded-2xl border-2 border-primary/30 bg-muted/30 pl-10 pr-4 py-3.5 text-sm font-medium outline-none tabular-nums placeholder:text-muted-foreground focus:border-primary transition-all duration-300" />
              </div>
            </div>
            <div className="space-y-3">
              <div className="flex items-center justify-between rounded-2xl bg-muted/30 border border-border/50 px-4 py-3.5">
              ...
              {simValue > 0 && (
                <div className="flex items-center justify-between rounded-2xl bg-destructive/8 border border-destructive/20 px-4 py-3.5">
          ...
          <div className="h-40">
            <ResponsiveContainer width="100%" height="100%">
              <BarChart data={entriesExitsData} margin={{ top: 5, right: 5, left: -15, bottom: 0 }}>
                ...
                <Tooltip contentStyle={{ background: "hsl(var(--card))", border: "1px solid hsl(var(--border))", borderRadius: "16px", fontSize: "11px" }} labelFormatter={(v) => `DIA ${v}`} formatter={(value: number, name: string) => [fmt(value), name === "entradas" ? "Entradas" : "Saídas"]} />
